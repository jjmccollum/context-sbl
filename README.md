...
- [ ] Following `biblatex-sbl`, define an `autocite` that handles list registry and inline-short decisions at a high level. Regarding list registry, entries in the `@inlexicon`, `@ancienttext`, and `@classictext` categories should not be added to the list when they are cited unless they are of type `listcite`. For inline-short variation, the `inline` setup should be used if a tag has not been encountered before, and the `short` setup should be used otherwise. Parenthetical citations can then be achieved with `autocite[left={\startparen},right={\stopparen}][...]` (although this could be more succinctly aliased as `parencite` for user convenience)
